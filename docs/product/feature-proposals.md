...
- **Deps/risks:** `claude --resume` semantics across truncation are unverified. The 2026-05-25 spike showed `claude --resume` is cwd-scoped (different from F4's blocker but related); for F5 specifically, the Director's cwd is `app.getPath('userData')` and stable, so resume-from-different-cwd doesn't apply. The truncation question is still open — could be cheap to answer with a follow-up spike. Codex side remains harder (different session model).
