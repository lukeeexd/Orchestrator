...
| Auto-update | `update-electron-app` against **`update.electronjs.org`** (Microsoft-operated, TLS-pinned to GitHub Releases) | v0.15.0 cut over to a self-hosted Cloudflare R2 feed; the R-H2 review reverted that decision (unsigned installer + remote-install surface = bad). A secondary signalling channel on Cloudflare Pages still polls `latest.json` for "new version available" notices but never auto-installs. Repo-private would require landing code-signing (H7) first. |
...
| Code signing | Deferred (H7 in 2026-05-18 review) | The installer is unsigned; SmartScreen warns on first run. Now also gates the "flip the repo private" follow-up: a self-hosted update feed (R2 / private repo) needs the installer signed first so the updater can verify it. |
| Repo location | `github.com/lukeeexd/Orchestrator`, **public** as of 2026-05-17 | History was rewritten before the privacy flip to scrub personal identifiers. Do not reintroduce real name / personal email anywhere. Flipping private is currently blocked on code signing (H7) — `update.electronjs.org` is public-repo-only, so any self-hosted feed needs signature verification before it can become primary. |
